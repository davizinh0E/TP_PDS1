...
void adicionarEstacao()
{
    ...
    do
    {
        valido = 1; // reseta antes de cada verificacao
        ...
        for (int i = 0; estacoes[totalEstacoes].nome[i] != '\0'; i++)
        {
            if (!isalpha(estacoes[totalEstacoes].nome[i]) && estacoes[totalEstacoes].nome[i] != ' ')
            {
                valido = 0;
                break;
            }
    ...
    do
    {
        ...
        for (int i = 0; estacoes[totalEstacoes].operador[i] != '\0'; i++)
        {
            if (!isalpha(estacoes[totalEstacoes].operador[i]) && estacoes[totalEstacoes].operador[i] != ' ')
            {
                valido = 0;
                break;
            }
        ...
        if (!valido)
            printf("Nome do operador invalido! Use apenas letras e espacos.\n");
    } while (!valido);
    ...
    do
    {
        ...
        {
            if (!isalpha(estacoes[totalEstacoes].sensor[i]) && estacoes[totalEstacoes].sensor[i] != ' ')
            {
                valido = 0;
                break;
            }
        ...
        if (!valido)
            printf("Sensor invalido! Use apenas letras e espacos.\n");
    } while (!valido);
    ...
    if (estacoes[totalEstacoes].leituras == NULL)
    { // teste de memoria disponível
